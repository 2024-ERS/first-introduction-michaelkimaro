...
author: "Han Olff modified by Michael"
date: "2024-08-29"
format: html
editor: 
  markdown: 
...
# there are large outliers due to likely data entry mistakes
FactCockleSize2 <- FactCockleSize |> dplyr::filter(thickness_mm<500) |>
  mutate(year=factor(year)) #make year factor

ggplot2::ggplot(data = FactCockleSize2, aes(length_mm,thickness_mm))+
  geom_point()
```
...
```{r}
model_line <-lm(thickness_mm~length_mm, data = FactCockleSize2)
summary(model_line)
...
between length and width is about the same for every year, we safely can use only length as a proxy for the biomass of an individual cockle
